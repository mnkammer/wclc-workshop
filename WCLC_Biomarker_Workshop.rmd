...
biomarkers <- list("Mayo", 
                   ...
                   "Dep.EntropyT",
                   "StrengthT")
roc_auc_data <- lapply(biomarkers, function(biomarker) compute_roc_auc(df[[biomarker]], df$y))

# Plot ROC curves
colors <- c("blue", "black", "red","green","brown","blue", "black", "red","green","brown","blue")
...
RAD_model <- lrm(y ~ (Strength) + (Flatness) +
                  (AntPost.Length) +
                  (L3.Distance) +
                  (Area.Density) +
                  (Volume.Density) +
...
\newpage
## Estimating Clinical Utility
```{r Clinical Utility}

```
\newpage
# Training on all available data
## Talk about how after being satisfied with validation, train on all available data for the most robust, generalized model
...
P0 <- rnorm(10000, mean = -1, sd = 1.5) # Case-negative population
...
\textbf{For Benign:} Observed = `r round(max(0,NRI_NoChange$obs_reclass_0),4)`, Expected = `r round(max(0,NRI_NoChange$exp_reclass_0),4)`, Net = `r round(max(0,NRI_NoChange$cNRI_0),4)`

\textbf{For Cancer:} Observed = `r round(max(0,NRI_NoChange$obs_reclass_1),4)`, Expected = `r round(max(0,NRI_NoChange$exp_reclass_1),4)`, Net = `r round(max(0,NRI_NoChange$cNRI_1),4)`

```{r Expected vs Observed Random}
...
\textbf{For Benign:} Observed = `r round(max(0,NRI_Random$obs_reclass_0),4)`, Expected = `r round(max(0,NRI_Random$exp_reclass_0),4)`, Net = `r round(max(0,NRI_Random$cNRI_0),4)`

\textbf{For Cancer:} Observed = `r round(max(0,NRI_Random$obs_reclass_1),4)`, Expected = `r round(max(0,NRI_Random$exp_reclass_1),4)`, Net = `r round(max(0,NRI_Random$cNRI_1),4)`
